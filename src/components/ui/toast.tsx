    ...
    <ToastContext.Provider value={{ toast: addToast }}>
      <ToastPrimitive.Provider swipeDirection="right">
        ...
        {toasts.map((t) => (
          ...
          >
            {icons[t.type]}
            <ToastPrimitive.Title className="flex-1 text-sm">{t.message}</ToastPrimitive.Title>
            <ToastPrimitive.Close className="transition-colors text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300">
              <X className="h-4 w-4" />
