...
function SheetContent({ className, children, title, ...props }: React.ComponentProps<typeof DialogPrimitive.Content> & { title?: string }) {
  return (
    <DialogPrimitive.Portal>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0 border-slate-100 dark:border-[#1e2d45]">
          {title ? (
            <DialogPrimitive.Title className="text-sm font-semibold text-slate-800 dark:text-slate-200">
              {title}
            </DialogPrimitive.Title>
          ) : (
            <DialogPrimitive.Title className="sr-only">Panel</DialogPrimitive.Title>
          )}
          <SheetClose className="rounded-lg p-1.5 transition-colors text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:text-slate-500 dark:hover:text-slate-300 dark:hover:bg-[#1e2540]">
            <X className="h-4 w-4" />
