  ...
  return (
    <DialogPortal>
      ...
      >
        {children}
        {showClose && (
          <DialogClose className="absolute right-4 top-4 rounded-lg p-1.5 transition-colors text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:text-slate-500 dark:hover:text-slate-300 dark:hover:bg-[#1e2540]">
            <X className="h-4 w-4" />
...
function DialogFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn('flex items-center justify-end gap-2 px-6 py-4 mt-6 border-t border-slate-100 dark:border-[#1e2d45]', className)}
      {...props}
    />
  );
}
...
function DialogDescription({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Description>) {
  return (
    <DialogPrimitive.Description
      className={cn('text-sm mt-1 text-slate-500 dark:text-slate-400', className)}
      {...props}
    />
  );
}
