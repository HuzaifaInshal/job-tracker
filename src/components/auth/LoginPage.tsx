...
import { Briefcase, ArrowLeft, CheckCircle2, Clock, Filter, Zap, Loader2 } from 'lucide-react';

const PERKS = [
  { icon: <Filter className="h-4 w-4" />, text: 'Filter & search all your applications instantly' },
  { icon: <Clock className="h-4 w-4" />, text: 'Full timeline for every application stage' },
  { icon: <Zap className="h-4 w-4" />, text: 'One-click status updates with auto-logging' },
  { icon: <CheckCircle2 className="h-4 w-4" />, text: 'Multi-channel tracking across all job boards' },
];
  ...
  return (
    <div className="min-h-screen bg-[#080b12] flex overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          <div className="py-12">
            <h2 className="font-display text-4xl xl:text-5xl font-800 tracking-[-0.025em] text-white leading-[1.1] mb-5">
              Your job search,
              <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">finally organised.</span>
            </h2>
            <p className="text-slate-400 text-base leading-relaxed mb-10 max-w-sm">
              Stop juggling spreadsheets. JobTrack gives you a single, powerful dashboard for every application in your pipeline.
            </p>
          ...
          <div className="mt-auto">
            <div className="rounded-xl border border-[#1e2d45] bg-[#0a0d18] overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#1a2235] bg-[#080b12]">
                ...
                <div className="text-[10px] text-slate-600 ml-2">jobtrack.app/dashboard</div>
              </div>
              {/* Rows */}
              {[
                { co: 'Stripe', role: 'Sr. Engineer', s: 'Accepted', sc: 'text-emerald-400', dot: 'bg-emerald-400' },
                { co: 'Figma', role: 'Product Designer', s: 'Pending', sc: 'text-amber-400', dot: 'bg-amber-400' },
                { co: 'Vercel', role: 'DevRel', s: 'Accepted', sc: 'text-emerald-400', dot: 'bg-emerald-400' },
              ].map((r) => (
                <div key={r.co} className="flex items-center gap-4 px-4 py-2.5 border-b border-[#1a2235]/40 last:border-0">
                  <span className={`flex items-center gap-1 text-[10px] font-medium ${r.sc} w-20 shrink-0`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${r.dot}`} />
                    {r.s}
                  </span>
                  <span className="text-[11px] font-medium text-slate-200 w-16">{r.co}</span>
                  <span className="text-[11px] text-slate-500">{r.role}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-slate-600 text-center mt-3">Your dashboard awaits</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 relative">
        ...
        <Link
          href="/"
          className="lg:hidden absolute top-6 left-6 flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-200 transition-colors"
        >
        ...
        <div className="relative z-10 w-full max-w-sm">
          ...
          <div className="gradient-border rounded-2xl p-6 shadow-2xl shadow-black/60">
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <span className="text-xs text-slate-600">Secured by Firebase Auth</span>
              <div className="flex-1 h-px bg-[#1e2d45]" />
            </div>
          </div>
